...
const TopGrossingMovies = async() => {
  ...
  const data  = await fetch('http://13.49.18.64/highest-grossing-movie') //fetch the data from fasytapi that is deployed in aws
  const jsonData = await data.json() // it get the json data
const movieData : MovieDataType[] = jsonData.data
  return (
    <>
    <h1 className=' text-center text-4xl font-bold my-4'>Top Grossing  Movie</h1>
